...
def student_extract_feature(model, opt, device):
    ...
    if opt.val_dataset == 'ucf101':
        train_dataset = UCF101ClipRetrievalDataset(opt.val_video_path,
                                                   opt.val_annotation_path,
                                                   16, 10, True, train_transforms)
    elif opt.val_dataset == 'hmdb51':
        train_dataset = HMDB51ClipRetrievalDataset(opt.val_video_path,
                                                   opt.val_annotation_path,
                                                   16, 10, True, train_transforms)
    ...
    np.save(path.join(opt.extracted_features_path, 'train_feature.npy'), features)
    np.save(path.join(opt.extracted_features_path, 'train_class.npy'), classes)
    ...
    if opt.val_dataset == 'ucf101':
        test_dataset = UCF101ClipRetrievalDataset(opt.val_video_path,
                                                  opt.val_annotation_path,
                                                  16, 10, False, test_transforms)
    elif opt.val_dataset == 'hmdb51':
        test_dataset = HMDB51ClipRetrievalDataset(opt.val_video_path,
                                                  opt.val_annotation_path,
                                                  16, 10, False, test_transforms)
    ...
    features = np.array(features).reshape(-1, 10, outputs['features'].shape[1])
    classes = np.array(classes).reshape(-1, 10)
    np.save(path.join(opt.extracted_features_path, 'test_feature.npy'), features)
    np.save(path.join(opt.extracted_features_path, 'test_class.npy'), classes)


def topk_retrieval(opt):
    """Extract features from test split and search on train split features."""
    print('Load local .npy files.')
    X_train = np.load(path.join(opt.extracted_features_path, 'train_feature.npy'))
    y_train = np.load(path.join(opt.extracted_features_path, 'train_class.npy'))
    X_train = np.mean(X_train,1)
    y_train = y_train[:,0]
    X_train = X_train.reshape((-1, X_train.shape[-1]))
    y_train = y_train.reshape(-1)

    X_test = np.load(path.join(opt.extracted_features_path, 'test_feature.npy'))
    y_test = np.load(path.join(opt.extracted_features_path, 'test_class.npy'))
    ...
    with open(path.join(opt.extracted_features_path, 'topk_correct.json'), 'w') as fp:
        json.dump(topk_correct, fp)

    return {k: topk_correct[k]/len(X_test) for k in ks}


def do_validation(model, opt, device, model_role='student'):
    if not path.exists(opt.extracted_features_path):
        makedirs(opt.extracted_features_path)
    if model_role == 'student':
        student_extract_feature(model, opt, device)
    # else:
    #     teacher_extract_feature(model, opt, device)
